(function() {
    ...
    const injectStyles = () => {
        ...
        s.innerHTML = `
            ...
            @media (max-width: 768px) { 
                ...
                #vton-float { padding: 10px 18px !important; font-size: 12px !important; bottom: 120px !important; right: 15px !important; }
            }
            ...
            /* UPDATED TO PREMIUM BLACK BOLD TEXT */
            .v-brand-logo { font-size:12px; font-weight:900; letter-spacing:1px; margin-bottom:15px; text-transform:uppercase; color: #000; display: inline-block; }
    ...
    const getGarmentCategory = (title) => {
        ...
        // Kurta and Saree are mapped to "one-pieces" to ensure full-body generation
    ...
    // =======================================================
    // NEW: Secret Analytics Sender
    // =======================================================
    const sendLog = (type) => {
        fetch(`${WORKER_URL}/log-event?client=${CLIENT_ID}`, {
            method: 'POST',
            body: JSON.stringify({ type: type, garment: window.location.href })
        }).catch(e => console.log(e)); // Silently fail if there's a network error
    };
    // =======================================================

    // THE ULTIMATE OMNI-SCANNER (Checks Code + URL, but ignores its own script!)
    const checkPage = () => {
        // 1. Read the actual URL at the top of the browser
        const fullUrl = window.location.href.toLowerCase();
        
        // 2. Read the HTML, but completely REMOVE all <script> tags so it doesn't detect its own Vercel link!
        const cleanHTML = document.documentElement.innerHTML.replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, '').toLowerCase();
        
        // 3. If "vton" is in the clean code OR in the URL, trigger it!
        const hasVtonTag = cleanHTML.includes('vton') || fullUrl.includes('vton');
        
        if (hasVtonTag || (window.VTON_CONFIG && window.VTON_CONFIG.force)) {
            showButton();
        } else {
            removeButton();
        }
    ...
    const showButton = () => {
        ...
        // ========================================
        // TRACK: Button Appeared! (Sent to Cloudflare)
        // ========================================
        sendLog("view");
        ...
        b.onclick = () => {
            if (b.dataset.state === "idle") {
                ...
                // ========================================
                // TRACK: Button Clicked! (Sent to Cloudflare)
                // ========================================
                sendLog("click");
    ...
    function showGuide() {
        ...
        // CHANGED DEFAULT TO 4 TRIALS
        let trials = localStorage.getItem("vton_trials") || 4;
            ...
            <div class="v-con">
                ...
                <div class="v-right">
                    ...
                    <ul class="v-list">
                        ...
                        <li>✅ <b>Jewelry/Accessories?</b> Ensure that body area is visible</li>
                    </ul>
